...
- **Added `--replace-domains` Flag**: New CLI argument accepts a comma-separated list of domains; any matching absolute URL is rewritten to a relative path during conversion. The `Makefile` exposes this via a `DOMAINS` variable defaulting to the srbyte/rodrigoamaya family of domains, eliminating the old `change-domain` target.
...
2. **[clean_text_to_markdown(text, replace_domains)](./blogger2md.py#L38)**:
   ...
   - Accepts an optional `replace_domains` list; each domain in the list is replaced with a relative path (`/`).

3. **[parse_blogger_xml(file_path, output_dir, include_comments, export_drafts, replace_domains)](./blogger2md.py#L265)**:
   ...
   - Passes `replace_domains` down to `clean_text_to_markdown` so domain rewriting happens at conversion time.
...
- **`make process`**: Parses the feed and rewrites the domains listed in `DOMAINS` to relative paths. Defaults to `feed.xml` if present, otherwise `feed.atom`.
- **`make process DOMAINS="a.com,b.com"`**: Override the domain rewrite list for this run.
- **`make process DOMAINS=""`**: Disable domain rewriting entirely.
